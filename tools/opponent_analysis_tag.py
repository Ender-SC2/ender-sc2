...
for tag in tags:
    ...
    while matches_address:
        ...
        ), f"Unexpected status_code returned from {matches_response.request.url}"
        ...
        for i in range(len(matches["results"])):
            ...
            match_participation_address = (
                f'https://aiarena.net/api/match-participations/?match={match["id"]}&bot={bot_id}'
            )
            match_participation_response = requests.get(match_participation_address, headers=auth)
            assert (
                match_participation_response.status_code == 200
