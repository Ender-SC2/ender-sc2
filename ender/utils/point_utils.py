...
async def closest_in_path(bot_ai: BotAI, point_list: list[Point2], reference: Point2, max_range: float) -> Point2:
    point_list = [point for point in point_list if
                  point != reference and reference.distance_to_point2(point) <= max_range]
    # NOTE: Offsetting the position trying to not be on top of buildings
    distances = [await bot_ai.client.query_pathing(point.towards(reference, 3), reference.towards(point, 3)) for point
                 in point_list]
