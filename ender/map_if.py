...
class Map_if(Common):
    ...
    def __step0(self):
        self.plan_timeout = 55 * self.seconds
        self.map = numpy.ndarray(shape=(200,200), dtype=self.Mapcolor)
        for right in range(200):
            for up in range(200):
                if (self.map_left <= right < self.map_right) \
                and (self.map_bottom <= up < self.map_top):
    ...
    def read_map(self, px, py) -> Mapcolor:
        if (self.map_left <= px < self.map_right) \
        and (self.map_bottom <= py < self.map_top):
