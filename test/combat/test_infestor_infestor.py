...
class TestInfestorInfestor(unittest.TestCase):
    def test_infestor_vs_infestor_1v1(self):
        winner = EnderTestBot(
            ...
            MultipleTestSetup([CreateUnitsTestSetup(UnitTypeId.INFESTOR, 1, Point2([-10, -10])), UpgradedSetup()]),
    ...
    def test_infestor_vs_infestor_1v6(self):
        ...
        loser = EnderTestBot(
            [AttackCenterBehavior(), AttackClosestEnemyBehavior()],
            MultipleTestSetup([CreateUnitsTestSetup(UnitTypeId.INFESTOR, 6, Point2([10, 10])), UpgradedSetup()]),
            No(HaveUnit()),
        )
        environment = TestEnvironment()
        environment.test(battle_maps[0], winner, loser)
