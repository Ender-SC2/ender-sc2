...
from ender.behavior.combat import FocusFireCombatBehavior, RepositionBehavior, \
    SidewardsBehavior, BackBehavior, ForwardBehavior, AttackCenterBehavior
...
class RoachRoachTests(unittest.TestCase):
    ...
    def test_hydra_vs_ravager_single_unit(self):
        winner = EnderTestBot([AttackCenterBehavior(), AttackClosestEnemyBehavior(), FocusFireCombatBehavior(),
                               SpellEffectDodgingBehavior()],
                              CreateUnitsTestSetup(UnitTypeId.HYDRALISK, 1, Point2([-10, -10])),
                              Any([No(HaveUnit()), AfterTime(60)]))
        loser = EnderTestBot([AttackCenterBehavior(), AttackClosestEnemyBehavior(), SpellCastingBehavior()],
    ...
    def test_hydra_vs_ravager_6_supply(self):
        winner = EnderTestBot([AttackCenterBehavior(), BackBehavior(), RepositionBehavior(), ForwardBehavior(),
                               SidewardsBehavior(), FocusFireCombatBehavior(), SpellEffectDodgingBehavior()],
